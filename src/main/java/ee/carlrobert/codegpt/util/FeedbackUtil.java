...
public class FeedbackUtil {
  private static final Logger LOG = Logger.getInstance(FeedbackUtil.class.getName());
  ...
  private static void sendFeedback(ChatCompletionParameters params, boolean liked) {
    ...
    assert url != null;
    var feedbackUrl = url.split("/chat")[0]
        + String.format("/requests/chat/%s/feedback", requestId);
